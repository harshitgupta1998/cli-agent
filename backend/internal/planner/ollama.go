...
Only plan terminal, shell, project, file, process, developer-tool, or command-history requests. For unrelated questions, return intent "unknown" with an empty command.
...
func normalizePlan(raw llmPlan, fallbackCWD string) (string, models.CommandPlan, error) {
	...
	if intent != models.IntentExecuteCommand && intent != models.IntentSearchHistory && intent != models.IntentUnknown {
