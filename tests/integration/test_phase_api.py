...
def test_phase_one_is_current_and_later_phases_are_mocked(api):
    status, payload = api.get("/v1/phases")

    assert status == 200
    assert payload["current_phase"] == "phase_1"
    ...
    assert phases["phase_2"]["status"] == "mocked"
    assert phases["phase_3"]["status"] == "mocked"
